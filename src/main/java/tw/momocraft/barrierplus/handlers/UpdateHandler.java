...
public class UpdateHandler {
    ...
    public UpdateHandler(){
       this.checkUpdates(BarrierPlus.getInstance().getServer().getConsoleSender(), false);
    }
    ...
    public void checkUpdates(CommandSender sender, boolean playerJoin) {
    	if (playerJoin) {
			if (this.updateNeeded(sender)) {
				...
				ServerHandler.sendMessage(sender, "&ehttps://www.spigotmc.org/resources/barrierplus.70510/history");
			}
			return;
		}
    	...
    	if (this.updateNeeded(sender) && this.updatesAllowed) {
    		...
    		ServerHandler.sendMessage(sender, "&ahttps://www.spigotmc.org/resources/barrierplus.70510/history");
